...
@register.inclusion_tag("pagelets/_render_content_area.html", takes_context=True)
    ...
    if page:
        ...
        for pagelet in page.pagelets.order_by("order"):
            ...
            content += render_to_string(
                "pagelets/_render_pagelet.html",
                {"include_links": False, "pagelet": pagelet},
...
def page_teaser(context, page, num_words):
    ...
    if page:
        ...
        if page.description:
            ...
            pagelets = page.get_area_pagelets("main", with_shared=False)
...
def create_page(context, link_text):
    ...
    admin_url = reverse(
        "admin:%s_%s_add" % (Page._meta.app_label, Page._meta.module_name)
    )
